...
@Component({
  selector: 'app-crud-act-economica',
  templateUrl: './crud-act-economica.component.html',
  styleUrls: ['./crud-act-economica.component.scss']
})
export class CrudActEconomicaComponent {
  ...
  Apis() {
    ...
    const resp = [];
    forkJoin([
      this.adminService.insertActividad_Economica(data).pipe(takeUntil(this.destroy$)),
    ]).subscribe(([data]: any) => {
  ...
  buscarUser() {
    ...
    const data = { "id": parseInt(this.idv) }
    this.adminService.get_Unique_Actividad_Economica(data).subscribe((response: any) => {
  ...
  actualizar() {
    ...
    this.adminService.update_Actividad_Economica(a).subscribe(() => {
      Swal.fire({
        position: 'top-end',
        icon: 'success',
  ...
  deleteUser(){
    ...
    this.adminService.delete_Actividad_Economica(data).subscribe((response: any) => {
      if (response.data == 1) {
        Swal.fire({
          position: 'top-end',
          icon: 'success',
