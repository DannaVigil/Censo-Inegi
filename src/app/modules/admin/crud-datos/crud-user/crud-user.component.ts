...
export class CrudUserComponent {
  ...
  @ViewChild('user', { read: ElementRef }) descripcionI!: ElementRef
  @ViewChild('descripcion', { read: ElementRef }) descripcion2I!: ElementRef
  ...
  constructor(private formBuilder: FormBuilder, private adminService: AdminService,
    ...
    this.formularioUpdate = this.formBuilder3.group({
      ...
      user:['', [Validators.required, Validators.maxLength(25), Validators.pattern(/^[A-Za-z]+$/)]],
    });
  ...
  buscarUser() {
    ...
    this.adminService.get_Unique_User(data).subscribe((response: any) => {
  ...
  mapUserUpdate(data: any) {
    this.descripcionI.nativeElement.value = data[0].useR_NAME;
    this.descripcionv = data[0].useR_NAME;
  ...
  actualizar() {
    console.log(this.descripcion2I.nativeElement.value)
    const b = this.descripcion2I.nativeElement.value;
    
    this.descripcionv =this.descripcionI.nativeElement.value;
    const a = { "id": parseInt(this.idv), "useR_NAME": this.descripcionv,"iD_ROL":b }
    this.adminService.update_Users(a).subscribe(() => {
      Swal.fire({
        position: 'top-end',
        icon: 'success',
  ...
  deleteUser(){
    ...
    this.adminService.delete_Users(data).subscribe((response: any) => {
      if (response.data == 1) {
        Swal.fire({
          position: 'top-end',
          icon: 'success',
