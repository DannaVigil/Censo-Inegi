...
@Component({
  selector: 'app-crud-tipo-vivienda',
  templateUrl: './crud-tipo-vivienda.component.html',
  styleUrls: ['./crud-tipo-vivienda.component.scss']
})
export class CrudTipoViviendaComponent {
  ...
  Apis() {
    ...
    const resp = [];
    forkJoin([
      this.adminService.insertTipoVivienda(data).pipe(takeUntil(this.destroy$)),
    ]).subscribe(([data]: any) => {
  ...
  buscarUser() {
    ...
    this.adminService.get_Unique_Tipo_Vivienda(data).subscribe((response: any) => {
  ...
  actualizar() {
    ...
    this.adminService.update_Tipo_Vivienda(a).subscribe(() => {
      Swal.fire({
        position: 'top-end',
        icon: 'success',
  ...
  deleteUser(){
    ...
    this.adminService.delete_Tipo_Vivienda(data).subscribe((response: any) => {
      if (response.data == 1) {
        Swal.fire({
          position: 'top-end',
          icon: 'success',
